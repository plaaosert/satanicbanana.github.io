function initBattles(areaData) {
	...
	// Reset level, xp, hp, mp to initial values
	...
	areaController.interval_object = setInterval(areaTick, 100);
}
...
function areaTick() {
	...
	expected = Math.floor((currentTime - areaController.time_begin) / 100);
	...
	if (fightController.current_turn > 0) {
		...
		if (areaController.next_encounter == -1) {
			...
			areaController.next_encounter = areaController.stepcount + randInt(min_next, max_next);
