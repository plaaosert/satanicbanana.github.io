...
multiRPrefixes = [
	["Sharpened ",     ["stats.atk", "*", sqrt, 0.1], [0, 5]],
	...
	["Bulky ",         ["stats.def", "+", standardItemScaling, 0.6], [0, 5]],
...
for (i=0; i<multiRPrefixes.length; i++) {
	...
	for (j=prefx[2][0]; j<=prefx[2][1]; j++) {
		weaponTexts[j][0].push([prefx[0], prefx[1]]);
	}
}
...
multiRSuffixes = [
	...
	[" (Damaged)",          ["stats.atk", "*", constant, -0.2], [0, 5]],
...
for (i=0; i<multiRSuffixes.length; i++) {
	...
	for (j=sufx[2][0]; j<=sufx[2][1]; j++) {
		weaponTexts[j][1].push([sufx[0], sufx[1]]);
	}
}