...
const map_configs = {
    ...
    "SNG": {
        id: "SNG",
        name: "Standard - No Gravity",
        desc: "A map with no features that (at zoom level 1) is the exact size of the canvas. Has no gravity.",
...
const entity_sprites = new Map([
    ...
    ["translocator", 1, "weapon/"],
    ["translocator_weapon", 1, "weapon/"],
    ["knife", 1, "weapon/"],
    ["knife_weapon", 1, "weapon/"],
...
let audios_list = [
    ...
    // https://pixabay.com/sound-effects/film-special-effects-drill-200178/
    ["drill_dash", "snd/drill_dash.mp3"],
    ["drill_burrow", "snd/drill_burrow.mp3"],
    ...
    ["wrench_metal", "snd/wrench_metal.mp3"],
...
class DrillRockParticle extends MovingParticle {
    constructor(position, rotation_angle, size, sprites, frame_speed, duration, looping, velocity, board, gravity, delay=0, render_behind=false) {
        super(position, rotation_angle, size, sprites, frame_speed, duration, looping, velocity, delay, render_behind);
    ...
    pass_time(time_delta) {
        ...
        if (result) {
            ...
            if (this.bouncemax > 0 && this.position > this.board.size.y) {
                this.velocity = new Vector2(this.velocity.x, this.velocity.y * 0.8);
...
class MovingFrictionTrailParticle extends MovingFrictionParticle {
    constructor(position, rotation_angle, size, sprites, frame_speed, duration, looping, velocity, friction, trail_col, trail_dur, board, delay=0, render_behind=false) {
        super(position, rotation_angle, size, sprites, frame_speed, duration, looping, velocity, friction, delay, render_behind);

        this.trail_col = trail_col;
        this.trail_dur = trail_dur;
...
class MovingFrictionGravityTrailParticle extends MovingFrictionTrailParticle {
    constructor(position, rotation_angle, size, sprites, frame_speed, duration, looping, velocity, friction, trail_col, trail_dur, board, gravity, delay=0, render_behind=false) {
        super(position, rotation_angle, size, sprites, frame_speed, duration, looping, velocity, friction, trail_col, trail_dur, board, delay, render_behind);
...
class BallParticle extends Particle {
    constructor(position, duration, colour, radius, opacity, aero_canvases, delay=0) {
        super(position, 0, 1, [null], 0, duration, false, delay, false);
...
class FadingBallParticle extends BallParticle {
    constructor(position, duration, colour, radius, opacity, aero_canvases, delay=0) {
    ...
    get_current_opacity() {
        let proportion = this.lifetime / this.duration;
        let opac = 1 - proportion;

        this.opacity = Math.max(0, opac) * this.original_opacity;
    }
...
class SqrFadingBallParticle extends FadingBallParticle {
    constructor(position, duration, colour, radius, opacity, aero_canvases, delay=0) {
        super(position, duration, colour, radius, opacity, aero_canvases, delay);
    }
    ...
    get_current_opacity() {
        ...
        this.opacity = Math.max(0, Math.pow(opac, 2)) * this.original_opacity;
    }
}

class MovingSqrFadingBallParticle extends SqrFadingBallParticle {
    constructor(position, duration, colour, radius, opacity, aero_canvases, velocity, delay=0) {
        super(position, duration, colour, radius, opacity, aero_canvases, delay);
...
class MovingFrictionSqrFadingBallParticle extends SqrFadingBallParticle {
    ...
    physics_step(time_delta) {
        ...
        objs.forEach(ball1 => {
            objs.forEach(ball2 => {
                ...
                if (ball1.ignore_ball_collisions || ball2.ignore_ball_collisions) {
                    return;
                }
    ...
    board.particles.forEach(particle => {
        ...
        if (particle.typ == PARTICLE_TYPE.TEXT) {
            ...
            balls_to_render.unshift({
                ...
                display: true,
                get_current_col: () => particle.colour,
            ...
            for (let i=0; i<times; i++) {
                ...
                if ((i+1) % coll_gran == 0) {
                    ...
                    board.balls.forEach(ball => {
                        ...
                        if (ball.invuln_duration <= 0 && ball.gets_hit) {
                            // OK to check for hits (not in invuln window)
                            board.balls.forEach(other => {
                                if (other.skip_physics || !other.collision || !other.can_hit)
                                    return;  // skip_physics balls should completely stop
                                ...
                                if (!ball.allied_with(other)) {
                                    ...
                                    let should_hit = colliding_weapons.length > 0;
                                    // now ask the ball if i should hit it
                                    should_hit = should_hit && ball.try_get_hit(other);
                    ...
                    board.balls.forEach(ball => {
                        if (ball.skip_physics || !ball.collision || !ball.gets_hit)
                            return;  // skip_physics balls should completely stop
                        ...
                        if (ball.invuln_duration <= 0) {
                            ...
                            intersecting_projectiles.forEach(projectile_data => {
                                if (this.invuln_duration > 0) {
                                    // LOL this is broken... TODO remove in next balance patch
                                    return;  // don't care if now invuln
                                }

                                let projectile = projectile_data[0];
                                
                                if (!ball.try_get_hit_by_projectile(projectile)) {
                                    return;
                                }
