...
async function load_audio_item(path, lazy=false) {
    return await load_audio_from_url(`https://plaao.net/${game_id}/assets/snd/${path}`, lazy);
}
...
function cleanup_finished_audio() {
    return audio_playing.filter(audio => !audio.ended);
}
